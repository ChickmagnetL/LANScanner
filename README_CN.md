...
*注：`tools/build/macos.sh` 现在会产出 `release/macos/LANScanner.app`，用于本机未签名的 macOS 测试；`tools/build/linux.sh` 会产出 `release/linux/LANScanner-x86_64.AppImage` 作为 Linux 发布物。请在各自原生平台上运行对应产物。*

首次在 macOS 上构建时，请确保系统里有 Xcode Command Line Tools、`python3` 以及 `curl` 或 `wget`。脚本会用它们从应用自带的 `ico` 图标派生临时 `.icns` 资源，并按需下载本地 Rust 工具链。首次在 Linux 上构建时，请确保系统里有 `python3` 以及 `curl` 或 `wget`，以便完成 AppImage 打包流程。
